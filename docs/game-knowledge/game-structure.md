...
- **`Audio/`**: This directory holds the game’s music and sound effects. The files are in an **bank format**. This is a special format created by FMOD. they are [editable](/docs/modding-tools/sound-editing.md) with an open source software
...
- **`Luts/`**: Contains list of LUTs (Look-Up Table) to map one set of colors to another, used in the photo mod. they can be [modified](/docs/modding-tools/lut-editing.md)
